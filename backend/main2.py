...
def get_script_path(module_type):
    ...
    mapping = {
        ...
        "ml_suite": os.path.join(SCRIPTS_PATH, "machine_learning", "ml.py"),  # Use the correct folder name
    }
...
def build_execution_order(modules):
    """Sorts modules by a predefined strict pipeline order."""
    type_order = [
        "scrape",
        ...
        "keyword_extraction",
        "rag_query",
        "ml_suite"  # ML Suite as the 12th module
    ]
    
    ordered_ids = [m['id'] for t in type_order for m in modules if m['type'] == t]
    ordered_ids += [m['id'] for m in modules if m['id'] not in ordered_ids]
...
def pipeline_worker(pipeline_id, modules, connections):
    ...
    try:
        ...
        for module_id in execution_order:
            ...
            logger.info(f"Looking for script at: {script_path}")  # <--- Add this line
...
def api_upload_ml_file(pipeline_id):
    try:
        ...
        # FIX: Save to machine_learning/uploads, not ml/uploads
